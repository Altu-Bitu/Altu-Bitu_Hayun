...
//다이내믹 프로그래밍 활용
//점화식 떠올리기 어렵다, 배열을 늘려서 0, 9도 동일한 처리 가능하도록

int easyStairNum(int n) {
	vector<vector<int>> dp(n + 1, vector<int>(12, 0)); // 자릿수는 1부터 시작하므로 n+1, 인덱스가 0~11까지 (실제 수 (0~9) +1인 인덱스에 저장, 실제 수 0은 [1]에 저장해서 dp[i-1][0] + dp[i][2], 실제 수 9는 [10]에 저장해서 dp[i-1][8] + dp[i-1][11])
	int ans = 0;

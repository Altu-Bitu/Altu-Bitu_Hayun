...
int maxMoney_1(int n, vector<ci> &counsel) {
	...
	for (int i = 1; i <= n; i++) {
		...
		if (i + counsel[i].first <= n + 1) {	//i번째 상담이 가능한 경우
			dp[i + counsel[i].first] = max(dp[i + counsel[i].first], dp[i] + counsel[i].second);	 //i번째의 상담 끝나는 날의 이익: 기존 값, (i번째 이익 + 상담 이익) 중 큰 값
		}
	}
...
int maxMoney_n(int n, vector<ci> &counsel) {
	...
	for (int i = n; i >= 1; i--) {
		...
		if (i + counsel[i].first <= n + 1) {	//i번째 상담이 가능한 경우
			dp[i] = max(dp[i], dp[i + counsel[i].first] + counsel[i].second);   //i번째 이익: 상담 안하는 경우 이익, (상담 끝나는 날의 이익 + 상담 이익) 중 큰 값
		}
	}
	return dp[1];
}

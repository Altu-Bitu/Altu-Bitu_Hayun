...
//얻을 수 있는 최대 이익 리턴하는 함수, start: 상담 시작 가능한 날, sum: 지금까지의 이익
void maxConsul(int n, vector<ci> &consulting, int start, int sum) {
	if (start > n) //상담 끝나는 날이 퇴사일을 넘는다면 -> 상담할 수 없는 경우
		return;
	ans = max(ans, sum); //현재 받아온 이익이 더 크면 ans로 저장
	
	for (int i = start; i < n; i++) // i번째 날을 상담하는 경우
		maxConsul(n, consulting, i + consulting[i].first, sum + consulting[i].second); //다음 검사: start = 현재 날짜 + 현재 상담 기간, sum = 지금까지의 이익 + 현재 상담 이익
}
...
[풀이]
1일부터 상담받는 경우에서 시작해서 할 수 있는 모든 경우의 수를 다 해보자
...
int main() {
	...
	for (int i = 0; i < n; i++)
		cin >> consulting[i].first >> consulting[i].second; //first: 상담 기간, second: 이익

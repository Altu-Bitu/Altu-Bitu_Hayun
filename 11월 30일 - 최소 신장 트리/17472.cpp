...
void dfs(int n, int m, int row, int col, int mark) {	//해당 구역의 섬을 mark로 표시하는 함수
...
int markIsland(int n, int m) {	//모든 섬을 탐색하며 구분 짓기 (mark 2부터)
	int mark = 1;
	for (int i = 0; i < n; i++) {
		for (int j = 0; j < m; j++) {
	...
	for (int i = 0; i < 4; i++) {	//상하좌우로 계속 다리 만들어보기
		...
		while (tr >= 0 && tr < n && tc >= 0 && tc < m) {	//좌표가 범위 내라면
			if (board[tr][tc]) {	//어디든 섬과 닿았다면
				...
				break;	//다른 섬에 닿았으면 더 이상 현재 방향으로 다리를 늘리지 않음 (?)
			}
			tr += dr[i];	//현재 방향으로 row 전진
			tc += dc[i];	//현재 방향으로 col 전진
			dist++;			//다리 길이 늘리기
		}
	}
}

priority_queue<tp, vector<tp>, greater<>> buildBridge(int n, int m) {	//가능한 모든 다리로 우선순위 큐 구성
	priority_queue<tp, vector<tp>, greater<>> pq;
	for (int i = 0; i < n; i++) {
		for (int j = 0; j < m; j++) {
			if (board[i][j])	//섬이라면
				findBridge(n, m, i, j, board[i][j], pq);	//다른 섬까지의 다리 -> 우선순위 큐
		}
	}
	return pq;	//가능한 모든 다리가 저장된 우선순위 큐
}
...
bool unionInput(int x, int y) {
	int xp = findParent(x);	//x의 루트
	int yp = findParent(y);	//y의 루트

	if (xp == yp)	//같은 집합이라면 유니온할 수 없음
		return false;
...
int kruskal(int v, priority_queue<tp, vector<tp>, greater<>> &pq) {	//크루스칼
	int cnt = 0, sum = 0;	//간선의 개수, 가중치의 합

	while (!pq.empty() && cnt < v - 1) {	//큐에 간선이 있고, 사용한 간선의 수가 v-1보다 적을 동안
		int cost = get<0>(pq.top());		//시작점으로부터의 가중치
		int x = get<1>(pq.top());			//정점 x좌표
		int y = get<2>(pq.top());			//정점 y좌표
